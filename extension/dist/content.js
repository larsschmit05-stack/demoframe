...
function toStack$1(node2) {
  ...
  if (node2.type === "document") {
    events2 = ["Document", CHILDREN$1, "DocumentExit"];
  } else if (node2.type === "root") {
    events2 = ["Root", CHILDREN$1, "RootExit"];
  } else {
    events2 = getEvents$1(node2);
  }
  return {
    eventIndex: 0,
    events: events2,
...
function initInputObserver({
  ...
  const events2 = sampling.input === "last" ? ["change"] : ["input", "change"];
  const handlers = events2.map(
  ...
  while (current && depth < 4) {
    ...
    if (parent) {
      ...
      if (siblings.length > 1) {
        const index2 = siblings.indexOf(current) + 1;
        parts.unshift(`${tag}:nth-of-type(${index2})`);
      } else {
        parts.unshift(tag);
      }
    } else {
...
const WARN_THRESHOLD = 2.5 * 1024 * 1024;
const STOP_THRESHOLD = 5 * 1024 * 1024;
class SizeGuard {
  estimatedSize = 0;
  warned = false;
  stopped = false;
...
function buildRecording(events2, interactiveElements, url, title) {
  const firstTimestamp = events2.length > 0 ? events2[0].timestamp : Date.now();
  const lastTimestamp = events2.length > 0 ? events2[events2.length - 1].timestamp : Date.now();
  const recording2 = {
    version: "0.1.0",
    url,
    title,
  ...
  const json = JSON.stringify(recording2);
  recording2.metadata.sizeBytes = new Blob([json]).size;
  return recording2;
}
...
class ElementDetector {
  ...
  start() {
    ...
    staticElements.forEach((el) => {
      this.registerElement(el, "static-scan");
    });
    this.clickHandler = (e) => {
...
function stopRecording() {
  ...
  detector?.stop();
  detector = null;
  const recording2 = buildRecording(
    events,
    detectedElements,
